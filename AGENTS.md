...
- `skills/engineering-os/` is the canonical guided skill; the plugin copy must stay
  byte-identical and `engineering-os.json` validation must stay deterministic.
...
- Use plain, easy English across project output. Use technical terms only when needed
  and explain them the first time.
